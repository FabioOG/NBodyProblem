...
void NBody::computePreset(const string& initFilename)
{
    body = {};

    // Init file read
    rapidcsv::Document initFile(initFilename);
    vector<long double> line = initFile.GetRow<long double>(0); // First line
    ...
    {
        line = initFile.GetRow<long double>(1+i);
        ...
        body.push_back(Body( vector3l(line[0],line[1],line[2]), vector3l(line[3],line[4],line[5]), line[6] ));
    }

    linkAll();

    compute(computationTime,skip);
}
...
void NBody::save(const string& filename)
{
